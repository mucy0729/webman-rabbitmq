...
abstract class FastBuilder implements BuilderInterface
{
    ...
    {
        $default = str_replace('\\', '.', get_called_class());
        $message['consumer_tag'] = $this->consumer_tag ?: ($this->queue_name ?: $default);

        $message['routing_key'] = $this->routing_key ?: ($this->queue_name ?: $default);

        $message['queue_name'] = $this->queue_name ?: $default;

        $message['exchange_name'] = $this->exchange_name ?: ($this->queue_name ?: $default);
